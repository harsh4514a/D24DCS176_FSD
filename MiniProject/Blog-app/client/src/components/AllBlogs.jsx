...
const AllBlogs = ({ isAdmin = true }) => {
    ...
    useEffect(() => {
        const getBlogs = async () => {
            setLoader(true);
            try {
                const query = isAdmin ? `/api/blog/get-all-blogs?page=1` : `/api/blog/get-all-blogs?userId=${user._id}&page=1`;
    ...
    const fetchBlogs = async (page = 2) => {
        try {
            const query = isAdmin ? `/api/blog/get-all-blogs?page=${page}` : `/api/blog/get-all-blogs?userId=${user._id}&page=${page}`;
